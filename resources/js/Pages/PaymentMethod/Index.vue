<template>
    ...
    <h2 class="font-bold text-2xl underline mb-2">Métodos de Pagamento</h2>
    <div>
        <!-- FILTERS -->
        <div class="xl:w-200">
            <form
            ...
            >
                ...
                <tbody>
                    <tr
                        ...
                        v-for="(value, index) in $page.props.payment_methods.data"
        ...
        <!-- ACTIONS -->
        <Paginate
            :pagination="$page.props.payment_methods"
            :onEachSize="3"
            @paginate="paginate"
        ></Paginate>
        <!-- END ACTIONS -->
...
function _create() {
    form.post(route("payment_method.create"), {
        onSuccess: () => {
            form.reset("name");
...
function _save() {
    form_update.patch(route("payment_method.update"), {
        onSuccess: () => {
            _cancelUpdate();
        },
    });
...
function _deleteAlert(id, payment_payment_method_count) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route("payment_method.delete", [id]));
        }
    });
}
